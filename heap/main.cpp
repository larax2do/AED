...
{
    GObject *box;
    HeapBinario<std::string> arbo;
    GtkWidget *img;
    GtkWidget *img2;
    GObject *gri;
    datis(GObject *b,GtkWidget *i,GtkWidget *i2,GObject *g,HeapBinario<std::string> &a):box{b},arbo{a},img{i},img2{i2},gri{g}{}
};
static void agregar (GtkWidget *widget, struct datis *data)
{
    ...
    gtk_image_set_from_file (GTK_IMAGE(data->img2),"vec.png");
...
int main (int argc, char *argv[])
{
    ...
    HeapBinario<std::string> arbolito;
    arbolito.mostrar();

    imagen = gtk_image_new_from_file ("ARBOL.png");
    grilla=gtk_builder_get_object(builder,"vbox1");
    gtk_box_pack_start(GTK_BOX(grilla),GTK_WIDGET(imagen),false,false,100);
    gtk_widget_show(GTK_WIDGET(imagen));

    imagen2 = gtk_image_new_from_file ("vec.png");
    grilla=gtk_builder_get_object(builder,"vbox1");
    gtk_box_pack_end(GTK_BOX(grilla),GTK_WIDGET(imagen2),false,false,50);
    gtk_widget_show(GTK_WIDGET(imagen2));
    //gtk_paned_add2(GTK_PANED(grilla),GTK_WIDGET(imagen));

    datis *datos=new datis{box1,imagen,imagen2,grilla,arbolito};
